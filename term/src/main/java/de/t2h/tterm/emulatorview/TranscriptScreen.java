...
/** A TranscriptScreen is a screen that remembers data that's been scrolled.
 *
...
class TranscriptScreen implements Screen {
    ...
    /** The width of the transcript, in characters.
     *
     * <p>Fixed at initialization.</p>
     */
    private int mColumns;

    /** The total number of rows in the transcript and the screen.
     *
     * <p>Fixed at initialization./p>
     */
     ...
     * @param screenRows  the height of just the screen, not including the transcript
     *
     *   The transcript holds lines that have scrolled off the top of the screen.
     */
    ...
    /** Draw a row of text.
     *
     * <p>Out-of-bounds rows are blank, not errors.</p>
     *
    ...
    public final void drawText (int row, Canvas canvas, float x, float y,
        ...
        try {
            ...
            // Attempt to draw on a finished transcript. Happens on Honeycomb (Android 3), which we do no
            // longer support.
            return;
        }
        ...
        while(column < columns && index < lineLen && line[index] != '\0') {
            ...
            if(style != lastStyle
                ...
                if(lastRunStart >= 0) {
                    renderer.drawTextRun(canvas, x, y, lastRunStart, runWidth,
                        line, lastRunStartIndex, index - lastRunStartIndex, lastSelectionStyle, lastStyle,
            ...
            index += incr;
            if(width > 1) {
                // We cannot draw two or more East Asian wide characters in the same run, because we need to
                // make each wide character take up two columns, which may not match the font's idea of the
                // character width,
                forceFlushRun = true;
            }
        }
        ...
        if(cx >= 0 && imeText.length() > 0) {
            ...
            try {
                renderer.drawTextRun(canvas, x, y, imePosition + imeLength, runWidth,
                    line, imePosition, runWidth - imePosition - imeLength, lastSelectionStyle, lastStyle,
                    -1, 0, 0, 0, 0);
            } catch(Throwable e) {
                // Ignore.
            }
    ...
    private String internalGetTranscriptText (
        GrowableIntArray colors, int selX1, int selY1, int selX2, int selY2
    ) {
    ...
    /** Get whether the line at this index is "basic" (contains only BMP characters of width 1). */
