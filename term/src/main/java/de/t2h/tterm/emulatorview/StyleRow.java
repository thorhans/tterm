 ...
 *
 * <p>We pack color and formatting information for a particular character into an int -- see the TextStyle
 * class for details. The simplest way of storing that information for a screen row would be to use an array
 * of int -- but given that we only use the lower three bytes of the int to store information, that
 * effectively wastes one byte per character -- nearly 8 KB per 100 lines with an 80-column transcript.</p>
 *
...
final class StyleRow {
    ...
    void set (int column, int style) {
        if(style == mStyle && mData == null) return;
        ensureData();
        setStyle(column, style);
    }

    int get (int column) {
        if(mData == null) return mStyle;
    ...
    int getSolidStyle () {
        if(mData != null) throw new IllegalArgumentException("Not a solid style");
        return mStyle;
    }
    ...
    void copy (int start, StyleRow dst, int offset, int len) {
        ...
        // There are other potentially fast cases, but let's just treat them all the same for simplicity.
        ensureData();
        dst.ensureData();
