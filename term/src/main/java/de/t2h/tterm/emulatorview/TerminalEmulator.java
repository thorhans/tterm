 ...
 * terminal, which in turn is an emulator for a subset of the Digital Equipment Corporation vt100 terminal.
...
class TerminalEmulator {
     ...
     *
     * <p>Ranges from 0 to MAX_ESCAPE_PARAMETERS-1. (Typically just 0 or 1.)</p>
     */
    private int mArgIndex;

    /** The number of parameter arguments.
     *
     * <p>This name comes from the ANSI standard for terminal escape codes.</p>
     */
     ...
     *  terminated.
     *
     * <p>Used when parsing a single character.</p>
     */
    ...
    /** Saved state of the cursor row.
     *
     * <p>Used to implement the save/restore cursor position escape sequences.</p>
     */
    private int mSavedCursorRow;

    /** Saved state of the cursor column.
     *
     * <p>Used to implement the save/restore cursor position escape sequences.</p>
     */
    ...
    /** This mask indicates 132-column mode is set.
     *
     * <p>(As opposed to 80-column mode.)</p>
     */
    ...
    /** This mask indicates that origin mode is set.
     *
     * <p>(Cursor addressing is relative to the absolute screen size, rather than the currently set top and
     * bottom margins.)</p>
     */
    private static final int K_ORIGIN_MODE_MASK = 1 << 6;

    /** This mask indicates that wraparound mode is set.
     *
     * <p>(As opposed to stop-at-right-column mode.)</p>
     */
    private static final int K_WRAPAROUND_MODE_MASK = 1 << 7;

    /** DECTCEM - This mask indicates that the cursor should be shown. */
    private static final int K_SHOW_CURSOR_MASK = 1 << 25;

    /** This mask is the subset of DecSet bits that are saved / restored by the DECSC / DECRC commands. */
    private static final int K_DECSC_DECRC_MASK =
        K_ORIGIN_MODE_MASK | K_WRAPAROUND_MODE_MASK;

    /** Holds multiple DECSET flags.
     *
     * <p>The data is stored this way, rather than in separate booleans, to make it easier to implement the
     * save-and-restore semantics. The various k*ModeMask masks can be used to extract and modify the
     * individual flags current states.</p>
     */
    private int mDecFlags;

    /** Saves away a snapshot of the DECSET flags.
     *
     * <p>Used to implement save and restore escape sequences.</p>
     */
    ...
    /** True if insert mode (as opposed to replace mode) is active.
     *
     * <p>In insert mode new characters are inserted, pushing existing text to the right.</p>
     */
    private boolean mInsertMode;

    /** An array of tab stops.
     *
     * <p>mTabStop[i] is true if there is a tab stop set for column i.</p>
     */
    ...
    /** The top margin of the screen, for scrolling purposes.
     *
     * <p>Ranges from 0 to mRows-2.</p>
     */
    private int mTopMargin;

    /** The bottom margin of the screen, for scrolling purposes.
     *
     * <p>Ranges from mTopMargin + 2 to mRows. (Defines the first row after the scrolling region.</p>
     */
    private int mBottomMargin;

    /** True if the next character to be emitted will be automatically wrapped to the next line.
     *
     * <p>Used to disambiguate the case where the cursor is positioned on column mColumns-1.</p>
     */
    private boolean mAboutToAutoWrap;

    /** The width of the last emitted spacing character.
     *
     * <p>Used to place combining characters into the correct column.</p>
     */
    private int mLastEmittedCharWidth = 0;

    /** True if we just auto-wrapped and no character has been emitted on this line yet.
     *
     * <p>Used to ensure combining characters following a character at the edge of the screen are stored in
     * the proper place.</p>
     */
    ...
    /** What is the current graphics character set.
     *
     * <p>[0] == G0, [1] == G1</p>
     */
     ...
     *
     *  <p>True if we're supposed to be drawing the special graphics.</p>
     */
    ...
    /** This is not accurate, but it makes the terminal more useful on small screens. */
     ...
     */
    public TerminalEmulator (TermSession session, TranscriptScreen screen, int columns, int rows,
        ColorScheme scheme
    ) {
        mSession = session;
        mMainBuffer = screen;
    ...
    public void updateSize (int columns, int rows) {
        ...
        if(columns <= 0) throw new IllegalArgumentException("rows:" + columns);
        if(rows <= 0)    throw new IllegalArgumentException("rows:" + rows);

        TranscriptScreen screen = mScreen;
        TranscriptScreen altScreen;
        altScreen = screen != mMainBuffer ? mMainBuffer: mAltBuffer;
        ...
        if(! fastResize) {
            // Save the character at the cursor (if one exists) and store an ASCII ESC character at the
            // cursor's location This is an epic hack that lets us restore the cursor later...
        ...
        if(! altFastResize) {
            ...
            // Unlike for the main transcript below, don't trim off trailing newlines -- the alternate
            // transcript lacks a cursor marking, so we might introduce an unwanted vertical shift in the
            // screen contents this way.
            char c, cLow;
            int colorOffset = 0;
            for(int i = 0; i <= end; i++) {
        ...
        for(int i = 0; i <= end; i++) {
            ...
            if(Character.isHighSurrogate(c)) {
                ...
                // We marked the cursor location with ESC earlier, so this the place to restore the cursor to.
        ...
        if(newCursorRow != -1 && newCursorCol != -1) {
            ...
            // Adjust for any scrolling between the time we marked the cursor location and now.
    ...
    private void process (byte b, boolean doUTF8) {
        ...
        if(doUTF8 && mUTF8Mode && handleUTF8Sequence(b))  return;
        ...
        case 7: // BEL
            // If in an OSC sequence, BEL may terminate a string; otherwise do nothing.
        ...
        default:
            ...
            case ESC_SELECT_LEFT_PAREN:                  doEscSelectLeftParen(b);       break;
            case ESC_SELECT_RIGHT_PAREN:                 doEscSelectRightParen(b);      break;
            case ESC_LEFT_SQUARE_BRACKET:                doEscLeftSquareBracket(b);     break; // CSI
            case ESC_LEFT_SQUARE_BRACKET_QUESTION_MARK:  doEscLSBQuest(b);              break; // CSI ?
            case ESC_PERCENT:                            doEscPercent(b);               break;
            case ESC_RIGHT_SQUARE_BRACKET:               doEscRightSquareBracket(b);    break;
            case ESC_RIGHT_SQUARE_BRACKET_ESC:           doEscRightSquareBracketEsc(b); break;
    ...
    private boolean handleUTF8Sequence (byte b) {
        ...
        if(mUTF8ToFollow > 0) {
            if((b & 0xc0) != 0x80) {
                // Not a UTF-8 continuation byte (doesn't begin with 0b10). Replace the entire sequence with
                // the replacement char.
                mUTF8ToFollow = 0;
                mUTF8ByteBuffer.clear();
                emit(UNICODE_REPLACEMENT_CHAR);

                // The Unicode standard (section 3.9, definition D93) requires that we now attempt to process
                // this byte as though it were the beginning of another possibly-valid sequence.
    ...
    private void doEscLSBQuest (byte b) {
        ...
        case 'h': // Esc [ ? Pn h - DECSET
            ...
            switch(arg) {
            case 1:     mKeyListener.setCursorKeysApplicationMode(true); break;
            case 47:
            case 1047:
            case 1049:  if(mAltBuffer != null) { mScreen = mAltBuffer; } break;
            }

            if(arg >= 1000 && arg <= 1003) mMouseTrackingMode = arg;
            break;
        ...
        case 'l': // Esc [ ? Pn l - DECRST
            ...
            switch(arg) {
            case 1:     mKeyListener.setCursorKeysApplicationMode(false); break;
            case 47:
            case 1047:
            case 1049:  mScreen = mMainBuffer; break;
            }

            if(arg >= 1000 && arg <= 1003) mMouseTrackingMode = 0;
            break;
        ...
        if((changedFlags & K_132_COLUMN_MODE_MASK) != 0) {
            // We don't actually set/reset 132 cols, but we do want the side effect of clearing the screen and
            // homing the cursor.
    ...
    private int getDecFlagsMask (int argument) {
        if(1 <= argument && argument <= 32) return (1 << argument);
        return 0;
    }
    ...
    private void doSelectCharSet (int charSetIndex, byte b) {
        int charSet;
        switch(b) {
        case 'A': charSet = CHAR_SET_UK;                  break;  // United Kingdom character set
        case 'B': charSet = CHAR_SET_ASCII;               break;  // ASCII set
        case '0': charSet = CHAR_SET_SPECIAL_GRAPHICS;    break;  // Special Graphics
        case '1': charSet = CHAR_SET_ALT_STANDARD;        break;  // Alternate character set
        case '2': charSet = CHAR_SET_ALT_SPECIAL_GRAPICS; break;
        default:  unknownSequence(b); return;
        }
    ...
    private void doEsc (byte b) {
        ...
        case 'D': doLinefeed();                          break;  // INDEX
        case 'E': setCursorCol(0); doLinefeed();         break;  // NEL
        case 'F': setCursorRowCol(0, mBottomMargin - 1); break;  // Cursor to lower-left corner of screen
        case 'H':  mTabStop[mCursorCol] = true;          break; // Tab set
        ...
        case 'N': unimplementedSequence(b);                   break;  // SS2
        case '0': unimplementedSequence(b);                   break;  // SS3
        case 'P': unimplementedSequence(b);                   break;  // Device control string
        case 'Z': sendDeviceAttributes();                     break;  // return terminal ID
        case '[': continueSequence(ESC_LEFT_SQUARE_BRACKET);  break;
        case '=': mbKeypadApplicationMode = true;             break;  // DECKPAM
        case ']': startCollectingOSCArgs();
                  continueSequence(ESC_RIGHT_SQUARE_BRACKET); break;  // OSC
        case '>' : mbKeypadApplicationMode = false;           break;  // DECKPNM
        default:   unknownSequence(b);                        break;
        }
    }
    ...
    private void doEscLeftSquareBracket (byte b) {
        ...
        case 'A': setCursorRow(Math.max(mTopMargin, mCursorRow - getArg0(1)));        break; // ESC [ Pn A - Cursor Up
        case 'B': setCursorRow(Math.min(mBottomMargin - 1, mCursorRow + getArg0(1))); break; // ESC [ Pn B - Cursor Down
        case 'C': setCursorCol(Math.min(mColumns - 1, mCursorCol + getArg0(1)));      break; // ESC [ Pn C - Cursor Right
        case 'D': setCursorCol(Math.max(0, mCursorCol - getArg0(1)));                 break; // ESC [ Pn D - Cursor Left
        case 'G': setCursorCol(Math.min(Math.max(1, getArg0(1)), mColumns) - 1);      break; // ESC [ Pn G - Cursor Horizontal Absolute
        case 'H': setHorizontalVerticalPosition();                                    break; // ESC [ Pn ; H - Cursor Position
        ...
        case 'K': // ESC [ Pn K - Erase in Line
            switch(getArg0(0)) {
            case 0:  blockClear(mCursorCol, mCursorRow, mColumns - mCursorCol); break; // Clear to right
            case 1:  blockClear(0, mCursorRow, mCursorCol + 1);                 break; // Erase start of line to cursor (including cursor)
            case 2:  blockClear(0, mCursorRow, mColumns);                       break; // Clear whole line
            default: unknownSequence(b);                                        break;
            }
            break;
        ...
        case 'T': unimplementedSequence(b);                                   break; // Mouse tracking
        case 'X': blockClear(mCursorCol, mCursorRow, getArg0(0));             break; // Erase characters
        case 'Z': setCursorCol(prevTabStop(mCursorCol));                      break; // Back tab
        case '?': continueSequence(ESC_LEFT_SQUARE_BRACKET_QUESTION_MARK);    break; // Esc [ ? -- start of a private mode set
        case 'c': sendDeviceAttributes();                                     break; // Send device attributes
        case 'd': setCursorRow(Math.min(Math.max(1, getArg0(1)), mRows) - 1); break; // ESC [ Pn d - Vert Position Absolute
        case 'f': setHorizontalVerticalPosition();                            break; // Horizontal and Vertical Position

        case 'g': // Clear tab stop
            switch(getArg0(0)) {
            case 0:  mTabStop[mCursorCol] = false; break;
            case 3:  for(int i = 0; i < mColumns; i++) {
                       mTabStop[i] = false;
                     }
                     break;
            default: break; // Specified to have no effect.
            }
            break;

        case 'h': doSetMode(true);  break; // Set Mode
        case 'l': doSetMode(false); break; // Reset Mode
        ...
        {
            // The top margin defaults to 1, the bottom margin (unusually for arguments) defaults to mRows.
            //
    ...
    private void selectGraphicRendition () {
        ...
        for(int i = 0; i <= mArgIndex; i++) {
            ...
            if(code == 0) {         mForeColor = mDefaultForeColor;        // reset
                                    mBackColor = mDefaultBackColor;
                                    mEffect = TextStyle.fxNormal;
            } else if(code == 1) {  mEffect |= TextStyle.fxBold;           // bold
            } else if(code == 3) {  mEffect |= TextStyle.fxItalic;         // italics, but rarely used as such;
                                                                           // "standout" (inverse colors) with TERM=screen
            } else if(code == 4) {  mEffect |= TextStyle.fxUnderline;      // underscore
            } else if(code == 5) {  mEffect |= TextStyle.fxBlink;          // blink
            } else if(code == 7) {  mEffect |= TextStyle.fxInverse;        // inverse
            } else if(code == 8) {  mEffect |= TextStyle.fxInvisible;      // invisible
            } else if(code == 10) { setAltCharSet(false);                  // exit alt charset (TERM=linux)
            } else if(code == 11) { setAltCharSet(true);                   // enter alt charset (TERM=linux)
            } else if(code == 22) { mEffect &= ~TextStyle.fxBold;          // normal color or intensity, neither bright, bold nor faint.
            } else if(code == 23) { mEffect &= ~TextStyle.fxItalic;        // italic off, but rarely used as such;
                                                                           // clears standout with TERM=screen.
            } else if(code == 24) { mEffect &= ~TextStyle.fxUnderline;     // underline off
            } else if(code == 25) { mEffect &= ~TextStyle.fxBlink;         // blink off
            } else if(code == 27) { mEffect &= ~TextStyle.fxInverse;       // inverse off
            } else if(code == 28) { mEffect &= ~TextStyle.fxInvisible;     // invisible off
            } else if(code >= 30 && code <= 37) {                          // foreground color
                mForeColor = code - 30;
            } else if(code == 38 && i+2 <= mArgIndex && mArgs[i+1] == 5) { // foreground 256 color
                int color = mArgs[i+2];
                if(checkColor(color))  mForeColor = color;
                i += 2;
            } else if(code == 39) { mForeColor = mDefaultForeColor;        // set default text color
                ...
                int color = mArgs[i+2];
                if(checkColor(color)) mBackColor = color;
                i += 2;
            } else if(code == 49) { mBackColor = mDefaultBackColor;        // set default background color
    ...
    private boolean checkColor (int color) {
        ...
        if(! result) {
            if(EmulatorDebug.LOG_UNKNOWN_ESCAPE_SEQUENCES) {
                Log.w(EmulatorDebug.LOG_TAG, String.format("Invalid color %d", color));
            }
        }
        return result;
    }
    ...
    private void doEscRightSquareBracket (byte b) {
        switch(b) {
        case 0x7:  doOSC();                                        break;
        case 0x1b: continueSequence(ESC_RIGHT_SQUARE_BRACKET_ESC); break; // Esc, probably start of Esc \ sequence
        default:   collectOSCArgs(b);                              break;
        }
    }
    ...
    private void doEscRightSquareBracketEsc (byte b) {
        ...
        case '\\': doOSC(); break;
        default:
            // The ESC character was not followed by a \, so insert the ESC and the current character in arg
            // buffer.
    ...
    private void doSetMode (boolean newValue) {
        ...
        switch(modeBit) {
        case 4:  mInsertMode = newValue;    break;
        default: unknownParameter(modeBit); break;
        }
    }
    ...
    private void sendDeviceAttributes () {
        // This identifies us as a DEC vt100 with advanced video options. This is what the xterm terminal
        // emulator sends.
        byte[] attributes = {
            // VT100
    ...
    private void parseArg (byte b) {
        if('0' <= b && b <= '9') {
            if(mArgIndex < mArgs.length) {
                ...
                int thisDigit = b - '0';
                mArgs[mArgIndex] = oldValue >= 0 ? oldValue * 10 + thisDigit : thisDigit;;
            }
            continueSequence();
        } else if(b == ';') {
            if(mArgIndex < mArgs.length) {
    ...
    private int nextOSCInt (int delimiter) {
        ...
        while(mOSCArgTokenizerIndex < mOSCArgLength) {
            byte b = mOSCArg[mOSCArgTokenizerIndex++];
            if((int) b == delimiter) {        break;
            } else if('0' <= b && b <= '9') { value = value < 0 ? 0: value * 10 + b - '0';
    ...
    private void logError (String errorType, byte b) {
        if(EmulatorDebug.LOG_UNKNOWN_ESCAPE_SEQUENCES) {
            StringBuilder buf = new StringBuilder();
            buf.append(errorType).append(" sequence ");
            buf.append(" EscapeState: ").append(mEscapeState);
            buf.append(" char: '").append((char) b).append("' (").append(b).append(")");
