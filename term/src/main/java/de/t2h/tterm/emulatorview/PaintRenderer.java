...
class PaintRenderer extends BaseTextRenderer {
    ...
    public void drawTextRun (
        Canvas canvas, float x, float y, int lineOffset,
        ...
        if(! invisible) {
            ...
            if(underline) mTextPaint.setUnderlineText(true);
            ...
            if(cursorVisible) {
                ...
                if(countBeforeCursor > 0){
                    canvas.drawText(text, /*index*/ index, /*count*/ countBeforeCursor,
                        /*x*/ left, /*y*/ textOriginY, mTextPaint);
                }

                // Text at cursor.
                mTextPaint.setColor(mPalette[TextStyle.ciCursorForeground]);
                canvas.drawText(text, /*index*/ cursorIndex, /*count*/ cursorIncr,
                    /*x*/ cursorX, /*y*/ textOriginY, mTextPaint);

                // Text after cursor.
                if(countAfterCursor > 0) {
                    mTextPaint.setColor(textPaintColor);
                    canvas.drawText(text, /*index*/ cursorIndex + cursorIncr, /*count*/ countAfterCursor,
                        /*x*/ cursorX + cursorWidth * mCharWidth, /*y*/ textOriginY, mTextPaint);
                }
            } else {
                canvas.drawText(text, index, count, left, textOriginY, mTextPaint);
            }

            if(bold)      mTextPaint.setFakeBoldText(false);
            if(underline) mTextPaint.setUnderlineText(false);
        }
    }
}
